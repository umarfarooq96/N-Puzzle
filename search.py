...
class SearchEngine:
    ...
    def initStats(self):
        ...
        self.smallestNotExplored = 10000000  # start with a inf so the next iteration is guarantee to pick new min
        ...
        while not self.open.empty():
            ...
            for succ in successors:
                ...
                if costbound is not None and (succ.gval > costbound[0] or
                    ...
                    self.smallestNotExplored = min(self.smallestNotExplored, succ.gval + succ_hval)
                    self.cost_bound_pruned += 1
                    if self.trace > 1:
